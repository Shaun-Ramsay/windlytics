...
export default function MapSelectionApp() {
  ...
  const runSimulation = async () => {
    ...
    for (const w of windmills) {
      ...
      if (!model) continue;
      try {
        const response = await fetch(`${process.env.BACKEND_URL}/generated-energy`, {
          method: "POST",
          ...
          >
            ...
            {windmills.map((w, i) => (
              <Marker
                ...
                aria-label={`Wind turbine #${i + 1} at latitude ${w.lat.toFixed(
                  3
                )}, longitude ${w.lng.toFixed(3)}`}
              >
                <Tooltip
                  direction="top"
                  offset={[0, -20]}
